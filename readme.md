...
4. __Save R objects on the fly.__ Sometimes (maybe when the genome quality is not good) some steps could fail, but results from other steps might be usful. In this case, we can generate a R image after each step for using outside of Genespace. 
5. Addition step to check genome id to make sure the same genome names are used across datasets.
